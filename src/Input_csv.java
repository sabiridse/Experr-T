...
public class Input_csv {
	...
	public void write_array (String number_term) throws Exception {
		...
		int except_term = bdw.get_except_term(number_term);//***получаю статус исключени€ терема
	
		if (except_term == 0){
		
				String time_lost_term = bdw.get_MAX_time_lost_term(number_term);//***получаю последее врем€ сигнала пропавшего терема		
	...
	public void writePir(String file) throws Exception{
		...
		FileInputStream points_info = new FileInputStream(file);//дл€ чтени€
        ...
        while (iterator_points_info.hasNext()) {
            ...
            rows_in_points_info++;        // эта переменна€ и будет кол-вом строк после полного цикла
        }
		...
		for (int i = 1; i < rows_in_points_info-1; i++) {
					...
					Row row_imp = sheet_poi_inf.getRow(i);//строка дл€ чтени€		
					
					
								try {	
										number_terminal = row_imp.getCell(0).getStringCellValue();//читаю данные из €чейки
...
//*****************************************************************************************************								
		...
		//***************************************************************************************************	
								try {
									...
									time_signal = row_imp.getCell(1).getStringCellValue();//читаю данные из €чейки
		    ...
		    //*************************************************************************************************		
								try {
									...
									time_pay = row_imp.getCell(2).getStringCellValue();//читаю данные из €чейки
								
								} catch (NullPointerException e) {time_pay = "01.11.2016 12:00";}
			//****************************************************************************************************					
								try {
									
									status_cash = row_imp.getCell(3).getStringCellValue();//читаю данные из €чейки
								} catch (NullPointerException e) {status_cash = "";}
									
								try {	
									
									status_print = row_imp.getCell(4).getStringCellValue();//читаю данные из €чейки
